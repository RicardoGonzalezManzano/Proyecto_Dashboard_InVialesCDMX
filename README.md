...
Proyecto/
    __init__.py         Declara el proyecto como paquete Python (expone clases principales)
    ...
    datos/
        ...
        revision_datos.ipynb                    Notebook de exploracion inicial del dataset
        viales_limpio.csv                       Limpio completo (no se sube)
        viales_limpio.parquet                   Version ligera (~9 MB) que usa el dashboard
...
Paso opcional - Regenerar datos desde el CSV original:
    python main.py
    python main.py --paso limpieza

El dashboard solo necesita datos/viales_limpio.parquet (incluido en el repo).
El CSV completo y el pipeline main.py solo hacen falta si quieres regenerar todo desde el original.
